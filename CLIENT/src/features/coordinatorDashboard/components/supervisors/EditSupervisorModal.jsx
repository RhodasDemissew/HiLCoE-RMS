...
export default function EditSupervisorModal({ open, supervisor, onClose, onSuccess, onToast }) {
  const [form, setForm] = useState({ first_name: "", middle_name: "", last_name: "", email: "", supervisor_id: "" });
  ...
  useEffect(() => {
    ...
    setSpecializations(Array.isArray(supervisor.specializations) ? supervisor.specializations : []);
    setError("");
    setSaving(false);
    setQuery("");
  ...
  async function handleSubmit(event) {
    ...
    if (!supervisor?._id || saving) return;
    ...
    try {
      ...
      const res = await api(`/supervisors/${supervisor._id}`, {
        method: "PATCH",
      ...
      onToast?.({ type: "success", message: "Supervisor updated." });
      onSuccess?.(data);
      onClose?.();
    } catch (err) {
      const message = err.message || "Failed to update supervisor";
      setError(message);
          ...
          <div className="flex min-h-full items-center justify-center px-2 sm:px-4 py-4 sm:py-10">
            <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-150" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <Dialog.Panel className="w-full max-w-2xl rounded-2xl sm:rounded-3xl bg-white p-4 sm:p-6 lg:p-8 shadow-2xl mx-2 sm:mx-0">
                ...
                <p className="mt-1 text-xs sm:text-sm text-[color:var(--neutral-500)]">Update details for {formatFullName(supervisor)}.</p>

                <form className="mt-4 sm:mt-6 space-y-3 sm:space-y-5" onSubmit={handleSubmit}>
                  <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
                    <div>
                      <label className="block text-xs sm:text-sm font-semibold text-[color:var(--neutral-800)]" htmlFor="edit-sup-first-name">
                        First Name
                      </label>
                      <input
                        id="edit-sup-first-name"
                        name="first_name"
                        required
                    ...
                    <div>
                      <label className="block text-xs sm:text-sm font-semibold text-[color:var(--neutral-800)]" htmlFor="edit-sup-middle-name">
                        Middle Name
                      </label>
                      <input
                        id="edit-sup-middle-name"
                        name="middle_name"
                    ...
                    <div>
                      <label className="block text-xs sm:text-sm font-semibold text-[color:var(--neutral-800)]" htmlFor="edit-sup-last-name">
                        Last Name
                      </label>
                      <input
                        id="edit-sup-last-name"
                        name="last_name"
                        required
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-[color:var(--neutral-800)]" htmlFor="edit-sup-email">
                        Email
                      </label>
                      <input
                        id="edit-sup-email"
                    ...
                    <div>
                      <label className="block text-xs sm:text-sm font-semibold text-[color:var(--neutral-800)]" htmlFor="edit-sup-id">
                        Supervisor ID
                      </label>
                      <input
                        id="edit-sup-id"
                  ...
                  <div>
                    <label className="block text-xs sm:text-sm font-semibold text-[color:var(--neutral-800)]">Specializations</label>
