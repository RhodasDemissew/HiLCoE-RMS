...
export default function ConfirmDeleteModal({ open, supervisor, loading = false, onCancel, onConfirm }) {
  const fullName = supervisor
    ? [supervisor.first_name, supervisor.middle_name, supervisor.last_name].filter(Boolean).join(' ') || supervisor.supervisor_id
    : '';

  return (
    <Transition show={open} as={Fragment}>
      <Dialog as="div" className="relative z-50" onClose={() => { if (!loading) onCancel?.(); }}>
